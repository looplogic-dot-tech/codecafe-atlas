...
def recognize_image(image: Image.Image, configs: tuple[str, ...]) -> str:
    """Run the requested OCR passes and release the rendered page immediately.

    A service page can require several different crops.  Keeping those PIL
    images alive until the next page used a large amount of RAM on long jobs.
    This helper makes the lifetime of every high-resolution render explicit.
    """
    texts: list[str] = []
    try:
        for config in configs:
            try:
                texts.append(
                    pytesseract.image_to_string(image, lang="spa+eng", config=config)
                )
            except Exception:
                try:
...
class AnalysisWorker(QObject):
    ...
    def run(self):
        pending_results: list[PageResult] = []

        def flush_results():
            if pending_results:
                self.page_batch_ready.emit(list(pending_results))
                    ...
                    for page_index in range(page_count):
                        ...
                        if not serial and self.use_ocr:
                            if pytesseract is None:
                                ...
                                header_ocr_text = recognize_image(image, tuple(configs))
                        ...
                        if self.document_mode == "service":
                            ...
                            if not report_date and self.use_ocr and pytesseract is not None:
                                ...
                                try:
                                    ...
                                    date_ocr_text = recognize_image(
                                        date_image, ("--psm 6", "--psm 7", "--psm 11")
                                    )
                                    report_date, report_date_confidence = extract_provider_report_date(
                                        date_ocr_text
                                    )
                                    ...
                                    if not report_date:
                                        ...
                                        value_ocr_text = recognize_image(
                                            value_image, ("--psm 7", "--psm 11")
                                        )
                                        report_date, report_date_confidence = extract_provider_report_date(
...
class PdfPage(QWidget):
    ...
    def start_analysis(self):
        ...
        # Blocking delivery provides backpressure: the worker cannot queue
        # hundreds of thumbnails while the graphical table is still drawing
        # earlier pages. Results arrive in small groups to reduce repaint cost.
        self.worker.page_batch_ready.connect(
            self.on_page_batch,
    ...
    def on_page_ready(self, result: PageResult):
        self._accept_page_result(result, refresh=True)

    def on_page_batch(self, batch: list[PageResult]):
        if not batch:
            return
        self.table.setUpdatesEnabled(False)
        try:
            for result in batch:
                self._accept_page_result(result, refresh=False)
    ...
    def _accept_page_result(self, result: PageResult, *, refresh: bool):
        if (
        ...
        self.append_table_row(result, adjust=refresh)
        if refresh:
            self.update_metrics()
    ...
    def append_table_row(self, result: PageResult, *, adjust: bool = True):
        row = self.table.rowCount()
        self.table.insertRow(row)
    ...
    def adjust_table_height(self):
        ...
        # Every result row is set to 108 px in populate_row(). Avoid the former
        # O(n²) full-table scan after every inserted page on large batches.
