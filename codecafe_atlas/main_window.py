...
from .auxiliary_tools_page import AuxiliaryToolsPage
...
from .technical_library_page import TechnicalLibraryPage
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.technical_library_page = TechnicalLibraryPage()
        ...
        self.data_page = DataPage(
            ...
            homologation_page=self.sync_compare_page,
        )
        self.auxiliary_tools_page = AuxiliaryToolsPage()
        self.counter_registry_page.bridge.equipmentChanged.connect(
            self.refresh_equipment_consumers
        )
        ...
        license_action = QAction("Licencia de uso / Software License", self)
        license_action.triggered.connect(lambda: show_use_agreement(self))
    ...
    def change_page(self, current, previous):
        ...
        elif key == "technical_library" and self.technical_library_page.root_folder:
            self.technical_library_page.reload_index()
    ...
    def open_page(self, key: str):
        if key == "sync_compare":
            self.data_page.show_homologation()
            key = "data"
    ...
    def refresh_equipment_consumers(self, _created: int = 0):
        """Refresh every view that consumes newly registered equipment."""
        self.directory_page.refresh()
        self.inventory_page.full_refresh()
        self.service_order_page.full_refresh()
