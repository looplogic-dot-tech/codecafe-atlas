...
class PdfLibraryPage(QWidget):
    ...
    def __init__(self):
        ...
        self.rename_button = QPushButton("Renombrar PDF…")
        self.rename_button.clicked.connect(self.rename_selected_pdf)
        self.rename_button.setEnabled(False)
        detail_buttons.addWidget(self.rename_button)
        ...
        self.rename_shortcut = QShortcut(QKeySequence(Qt.Key.Key_F2), self.file_list)
        self.rename_shortcut.activated.connect(self.rename_selected_pdf)
    ...
    def rename_selected_pdf(self) -> None:
        """Rename the selected file on disk without moving it or overwriting another PDF."""
        ...
        if path.suffix.lower() != ".pdf" or not self._path_is_inside_root(path):
            QMessageBox.warning(
                ...
                "Atlas solo puede renombrar PDFs ubicados dentro de la carpeta seleccionada.",
            )
            return
        if not path.exists():
        ...
        if not new_stem:
            QMessageBox.warning(self, "Nombre no válido", "El nombre del PDF no puede quedar vacío.")
            return
        if re.search(r'[<>:"/\\|?*\x00-\x1f]', new_stem) or new_stem.endswith((" ", ".")):
            QMessageBox.warning(
                self,
                "Nombre no válido",
                "El nombre contiene caracteres no permitidos o termina con un punto o espacio.",
            )
            return

        new_path = path.with_name(f"{new_stem}.pdf")
        if new_path == path:
            return
        if new_path.exists():
            QMessageBox.warning(
                ...
                f"Ya existe un PDF llamado:\n{new_path.name}\n\nAtlas no sobrescribió ningún archivo.",
            )
            return
        ...
        old_zoom = self.zoom
        old_fit_width = self.fit_width_enabled
        self.close_document()
        try:
        ...
        self._reload_index(reset_duplicate_state=True)
        self.load_document(new_path)
        if self.document is not None:
            self.current_page = min(old_page, self.document.page_count - 1)
            self.rotation = old_rotation
            self.zoom = old_zoom
            self.fit_width_enabled = old_fit_width
            self.render_page()
