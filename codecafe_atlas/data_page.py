...
from PySide6.QtWidgets import (
    QFileDialog, QLabel, QMessageBox, QPushButton, QTabWidget, QVBoxLayout, QWidget,
)
...
class DataPage(QWidget):
    def __init__(self, database: Database, refresh_callback=None, homologation_page=None):
        ...
        layout.addWidget(page_header("Administrar datos","Importa, respalda, exporta y homologa las bases de Atlas."))

        self.tabs = QTabWidget()
        layout.addWidget(self.tabs, 1)
        database_tab = QWidget()
        database_layout = QVBoxLayout(database_tab)
        database_layout.setContentsMargins(12, 16, 12, 12)
        self.tabs.addTab(database_tab, "Base de datos")

        self.current_path=QLabel(f"Base activa:\n{database_path()}"); self.current_path.setWordWrap(True); self.current_path.setObjectName("dataPath"); database_layout.addWidget(self.current_path)
        ...
        database_layout.addSpacing(12)
        for w in (import_button,export_button,backup_button,reset_button,location_button): database_layout.addWidget(w)
        note=QLabel("La previsualización no escribe nada. La exportación Excel genera una fila editable por equipo y oculta los campos técnicos de la base. Reemplazar crea un respaldo, filtra duplicados de serie y verifica los conteos antes de activar la base."); note.setWordWrap(True); note.setObjectName("pageSubtitle"); database_layout.addSpacing(8); database_layout.addWidget(note); database_layout.addStretch(1)
        if homologation_page is not None:
            self.tabs.addTab(homologation_page, "Homologación")
