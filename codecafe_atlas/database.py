...
class Database:
    ...
    def _equipment_for_hostname(
        ...
        normalized = self._normalize_equipment_identifier(hostname)
        if not normalized:
            return None
        rows = connection.execute(
            "SELECT id, serial_number, hostname FROM atlas_equipment WHERE trim(hostname)<>''"
        ).fetchall()
        for row in rows:
            if self._normalize_equipment_identifier(row["hostname"]) == normalized:
                return row
        return None
    ...
    def missing_counter_equipment(
        ...
        with self.connect() as connection:
            for record in records:
                ...
                normalized = self._normalize_equipment_identifier(serial_number)
                if not normalized or normalized in missing:
                    continue
                if self._equipment_id_for_serial(connection, serial_number) is not None:
                    continue
                missing[normalized] = {
    ...
    @staticmethod
    def _counter_equipment_brand(model: str) -> str:
        """Infer only an explicit leading manufacturer; never invent a brand."""
        ...
        known = {
            ...
            "hp": "HP", "kyocera": "Kyocera", "lexmark": "Lexmark",
            "ricoh": "Ricoh", "samsung": "Samsung", "sharp": "Sharp",
            "xerox": "Xerox",
        }
        return known.get(first.casefold(), "")
    ...
    def _link_unassigned_counter_readings(
        ...
        """Attach earlier orphan readings after their equipment is registered."""
        normalized = self._normalize_equipment_identifier(serial_number)
        if not normalized:
            return 0
        rows = connection.execute(
            ...
            WHERE equipment_id IS NULL AND trim(serial_snapshot) <> ''
            """
        ).fetchall()
        reading_ids = [
            int(row["id"])
            for row in rows
            if self._normalize_equipment_identifier(row["serial_snapshot"]) == normalized
        ]
        if not reading_ids:
            return 0
        marks = ",".join("?" for _ in reading_ids)
        connection.execute(
            f"UPDATE atlas_counter_readings SET equipment_id=?, updated_at=CURRENT_TIMESTAMP WHERE id IN ({marks})",
            [equipment_id, *reading_ids],
        )
        return len(reading_ids)
    ...
    def save_counter_records(
        ...
        *,
        ...
        equipment_created = 0
        historical_linked = 0
        hostnames_completed = 0
        hostnames_preserved = 0
        hostname_conflicts: list[dict[str, str]] = []

        with self.connect() as connection:
            selected_dependency_id: int | None = None
            if register_missing:
                try:
                    selected_dependency_id = int(dependency_id) if dependency_id is not None else None
                except (TypeError, ValueError):
                    selected_dependency_id = None
                if selected_dependency_id is None or connection.execute(
                    "SELECT 1 FROM atlas_dependencies WHERE id=? AND active=1",
                    (selected_dependency_id,),
                ).fetchone() is None:
                    raise ValueError(
                        "Selecciona una dependencia válida antes de registrar los equipos nuevos."
                    )
            ...
            for record in records:
                ...
                equipment_was_created = False

                if equipment_id is None and register_missing and serial_number:
                    hostname_for_new = ""
                    if hostname:
                        hostname_owner = self._equipment_for_hostname(connection, hostname)
                        if hostname_owner is None:
                            hostname_for_new = hostname
                        else:
                            hostname_conflicts.append({
                                "serial_number": serial_number,
                                "hostname": hostname,
                                "owner_serial": str(hostname_owner["serial_number"] or ""),
                            })
                    cursor = connection.execute(
                        """
                        INSERT INTO atlas_equipment (
                            dependency_id, equipment_type, brand, model,
                            serial_number, inventory_number, ip_address,
                        ...
                        (
                            selected_dependency_id,
                            "Impresora",
                            self._counter_equipment_brand(model),
                            model,
                            serial_number,
                            hostname_for_new,
                    ...
                    equipment_was_created = True
                    equipment_created += 1
                    if hostname_for_new:
                        hostnames_completed += 1
                    historical_linked += self._link_unassigned_counter_readings(
                        connection,
                        equipment_id,
                        serial_number,
                    )

                if equipment_id is not None and hostname and not equipment_was_created:
                    equipment = connection.execute(
                        "SELECT id,serial_number,hostname FROM atlas_equipment WHERE id=?",
                        (equipment_id,),
                    ).fetchone()
                    current_hostname = str(equipment["hostname"] or "").strip() if equipment else ""
                    if current_hostname:
                        if (
                            self._normalize_equipment_identifier(current_hostname)
                            != self._normalize_equipment_identifier(hostname)
                        ):
                            hostnames_preserved += 1
                    else:
                        hostname_owner = self._equipment_for_hostname(connection, hostname)
                        if hostname_owner is not None and int(hostname_owner["id"]) != equipment_id:
                            conflict = {
                                "serial_number": serial_number,
                                "hostname": hostname,
                                "owner_serial": str(hostname_owner["serial_number"] or ""),
                            }
                            if conflict not in hostname_conflicts:
                                hostname_conflicts.append(conflict)
                        else:
                            connection.execute(
                                "UPDATE atlas_equipment SET hostname=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND trim(hostname)=''",
                                (hostname, equipment_id),
                            )
                            hostnames_completed += 1
        ...
        return {
            ...
            "hostnames_completed": hostnames_completed,
            "hostnames_preserved": hostnames_preserved,
