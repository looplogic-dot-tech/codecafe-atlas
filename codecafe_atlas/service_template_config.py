...
class ServiceTemplateConfigDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        choose_custom = QPushButton("Cargar plantilla Excel propia…")
        ...
        saved_rows = self._settings.get("field_mappings")
        required_saved = self._settings.get("required_placeholders")
        if not isinstance(required_saved, list):
            required_saved = list(SERVICE_REQUIRED_PLACEHOLDERS)
        ...
        def add_initial_row(
            field: str, token: str, cells: Any = "", required: bool = False,
            source_token: str = "",
        ) -> None:
            row = self.table.rowCount()
            self.table.insertRow(row)
            field_item = QTableWidgetItem(str(field or ""))
            field_item.setData(Qt.ItemDataRole.UserRole, str(source_token or token or ""))
            self.table.setItem(row, 0, field_item)
            self.table.setItem(row, 1, QTableWidgetItem(str(token or "")))
            if isinstance(cells, list):
                cells = ", ".join(str(cell) for cell in cells if str(cell).strip())
            self.table.setItem(row, 2, QTableWidgetItem(str(cells or "")))
            required_item = QTableWidgetItem("Sí" if required else "No")
            required_item.setFlags(required_item.flags() | Qt.ItemFlag.ItemIsUserCheckable | Qt.ItemFlag.ItemIsEditable)
            required_item.setCheckState(Qt.CheckState.Checked if required else Qt.CheckState.Unchecked)
        ...
        if isinstance(saved_rows, list) and saved_rows:
            for saved in saved_rows:
                ...
                add_initial_row(
                    ...
                    str(saved.get("placeholder") or ""),
                    saved.get("cells") or "",
                    bool(saved.get("required")),
                    str(saved.get("source_placeholder") or saved.get("placeholder") or ""),
                )
        else:
            for token in tokens:
        ...
        add_mapping = QPushButton("＋ Añadir campo")
        remove_mapping = QPushButton("Eliminar fila seleccionada")
        ...
        add_mapping.clicked.connect(self.add_mapping_row)
        remove_mapping.clicked.connect(self.remove_selected_mapping_rows)
        ...
        elif self.managed_template_path.exists():
            self._selected_source = self.managed_template_path
            self.template_edit.setText(str(self.managed_template_path))
            self.inspect_selected_template()
        elif self.included_template.exists():
            self.select_included()
        else:
            self.status.setText(
                "No hay plantilla incluida disponible. Usa ‘Cargar plantilla Excel propia…’ para seleccionar la única plantilla activa."
            )
    ...
    def add_mapping_row(self) -> None:
        ...
        required_item = QTableWidgetItem("No")
        required_item.setFlags(required_item.flags() | Qt.ItemFlag.ItemIsUserCheckable | Qt.ItemFlag.ItemIsEditable)
        required_item.setCheckState(Qt.CheckState.Unchecked)
        self.table.setItem(row, 3, required_item)
        self.table.setCurrentCell(row, 0)
        self.table.editItem(self.table.item(row, 0))

    def remove_selected_mapping_rows(self) -> None:
        rows = sorted({index.row() for index in self.table.selectedIndexes()}, reverse=True)
        for row in rows:
            self.table.removeRow(row)

    def select_included(self) -> None:
        if not self.included_template.exists():
            QMessageBox.warning(
                self, "Plantilla incluida no disponible",
                "La plantilla incluida no existe. Selecciona una plantilla propia; Atlas no requiere archivos históricos."
            )
            return
    ...
    def _current_required_placeholders(self) -> set[str]:
        required: set[str] = set()
        for row in range(self.table.rowCount()):
            token_item = self.table.item(row, 1)
            required_item = self.table.item(row, 3)
            token = token_item.text().strip().upper() if token_item else ""
            if token and required_item and required_item.checkState() == Qt.CheckState.Checked:
                required.add(token)
        return required
    ...
    def inspect_selected_template(self) -> None:
        ...
        try:
            ...
            found, missing, unknown = validate_service_template(path, self._current_required_placeholders())
    ...
    def _field_mappings(self) -> list[dict[str, Any]]:
        mappings: list[dict[str, Any]] = []
        seen: set[tuple[str, tuple[str, ...]]] = set()
        for row in range(self.table.rowCount()):
            field_item = self.table.item(row, 0)
            token_item = self.table.item(row, 1)
            cells_item = self.table.item(row, 2)
            required_item = self.table.item(row, 3)
            field = field_item.text().strip() if field_item else ""
            token = token_item.text().strip().upper() if token_item else ""
            raw = cells_item.text().strip() if cells_item else ""
            source_token = str(field_item.data(Qt.ItemDataRole.UserRole) or "").strip() if field_item else ""
            if not source_token:
                reverse_labels = {label.casefold(): key for key, label in FIELD_LABELS.items()}
                source_token = reverse_labels.get(field.casefold(), "")
            if not source_token and token in SERVICE_SUPPORTED_PLACEHOLDERS:
                source_token = token
            required = bool(required_item and required_item.checkState() == Qt.CheckState.Checked)
            if not any((field, token, raw, required)):
                continue
            if not token:
                raise ValueError(f"La fila {row + 1} no tiene placeholder.")
            if not re.fullmatch(r"\{\{[A-Z0-9_]+\}\}", token):
                raise ValueError(f"Placeholder no válido en la fila {row + 1}: {token}")
            cells = normalize_cell_list(raw) if raw else []
            key = (token, tuple(cells))
            if key in seen:
                continue
            seen.add(key)
            mappings.append({
                "field": field or FIELD_LABELS.get(token, token),
                "placeholder": token,
                "cells": cells,
                "required": required,
                "source_placeholder": source_token,
            })
        return mappings

    @staticmethod
    def _cell_map_from_mappings(mappings: list[dict[str, Any]]) -> dict[str, list[str]]:
        result: dict[str, list[str]] = {}
        for mapping in mappings:
            token = str(mapping.get("placeholder") or "").strip()
            cells = mapping.get("cells") or []
            if token and cells:
                result.setdefault(token, [])
                for cell in cells:
                    if cell not in result[token]:
                        result[token].append(cell)
        return result
    ...
    def accept_configuration(self) -> None:
        ...
        try:
            ...
            cell_map = self._cell_map_from_mappings(mappings)
            required_placeholders = {
                str(item["placeholder"]).strip() for item in mappings if item.get("required")
            }
            found, missing, unknown = validate_service_template(source, required_placeholders)
            ...
            payload = {
                ...
                "required_placeholders": sorted(required_placeholders),
            }
