...
echo ============================================================
echo CodeCafe Atlas v1.0.24.26 - reproducible Windows build
echo ============================================================
...
rem then copies dist back and creates release\CodeCafe_Atlas_v1.0.24.26_Windows_x64.zip.
".venv\Scripts\python.exe" build_windows_release.py
if errorlevel 1 goto :error

echo.
echo Compilacion v1.0.24.26 terminada y validada.
echo Distro: dist\CodeCafe-Atlas
echo ZIP portable: release\CodeCafe_Atlas_v1.0.24.26_Windows_x64.zip
echo Paquete de actualizacion opcional:
echo .venv\Scripts\python.exe make_update_package.py --dist dist\CodeCafe-Atlas --version 1.0.24.26 --platform windows --architecture x86_64
pause
exit /b 0
