...
- La actualización de `Ubicación de Equipo` consulta todos los equipos registrados en Atlas, aunque todavía no tengan una lectura histórica de contadores.
- En modo reemplazo, `P` debe coincidir exactamente con el nombre canónico de la dependencia: también se corrigen abreviaturas, acentos, mayúsculas y redacciones diferentes.
- Los equipos sin lectura pueden actualizar su ubicación, pero jamás generan contadores ficticios ni ceros.

## v1.0.24.42 — mapeo de ubicación y navegación de datos

- Al activar la actualización de ubicación, `Ubicación de Equipo` queda reservada y se excluye del mapeo de contadores. Un valor manual `P,R,S,V,W` se normaliza a `R,S,V,W`.
- Homologación se encuentra ahora dentro de **Administrar datos**, como pestaña propia.

## v1.0.24.41 — ubicación de equipo desde Atlas

- Añade una opción desactivada por omisión para completar `Ubicación de Equipo` con la dependencia registrada en Atlas.
- Detecta el encabezado dinámicamente; en la hoja maestra actual corresponde a la columna `P`.
- Un segundo control permite autorizar también el reemplazo de ubicaciones existentes.
- La vista previa indica el valor existente, la dependencia disponible y la acción propuesta.
- Las sustituciones quedan documentadas en el CSV de discrepancias.
- Protege fórmulas, verifica la copia resultante y nunca modifica la hoja maestra original.

## v1.0.24.40 — corrección editable de series discrepantes

- Permite corregir desde la vista previa un número de serie marcado como discrepante.
- La serie puede editarse con doble clic o `F2`, o mediante `Corregir serie seleccionada…`.
- Después de editarla, Atlas repite el análisis y exige nuevamente una coincidencia exacta.
- La corrección se aplica únicamente al lote actual; no reescribe silenciosamente el historial de la base de datos.
- Permite volver a editar o deshacer una corrección antes de generar la copia.

## v1.0.24.39 — lector PDF local en Registro de contadores

- Integra PDF.js 3.11.174 y su proceso de lectura dentro de Atlas.
- El Registro de contadores ya no depende de una conexión a CDN para abrir PDFs.
- Cuando un PDF realmente está dañado, protegido o no disponible, el aviso muestra su nombre y la causa detectada.
- Mantiene como máximo dos PDFs abiertos simultáneamente para procesar lotes grandes sin acumular todos los documentos en RAM.
- Conserva el OCR nativo, la revisión, el historial y la generación de reportes existentes.

## v1.0.24.38 — insertador en paneles de trabajo

- Divide el Insertador inteligente en un panel izquierdo de configuración y un panel derecho de resultados.
- Dedica la mayor parte de la pantalla a la tabla, el detalle y las discrepancias relevantes.
- Permite redimensionar ambos paneles arrastrando el separador y recuerda la proporción elegida.
- Compacta las opciones de configuración sin alterar la lógica de lectura, inserción, estados ni protección de fórmulas.

## v1.0.24.37 — altas de series faltantes con protección contra errores

- Distingue el total de lecturas históricas del número de series únicas; por ejemplo, `200 lecturas / 195 series únicas`.
- Puede crear en la copia nuevas filas para series ausentes de la hoja maestra.
- Cada fila nueva escribe únicamente datos comprobados: Serie, `LOCALIDAD=Torreón` y los contadores seleccionados.
- Detecta la columna `Estatus/Estado del equipo`; en la hoja actual corresponde a `J`.
- Cuando el valor efectivo de al menos un contador es mayor que cero, cambia `Almacenada` o vacío a `En Operación`.
- Un equipo cuyos contadores permanezcan en cero conserva su estado anterior.
- Antes de crearla, busca series existentes con una posible diferencia de captura o transposición.
- Una serie parecida queda bloqueada y señalada para revisión; nunca se corrige ni se fusiona automáticamente.
- Después de revisar la advertencia, el usuario puede autorizar expresamente el alta y Atlas la documenta como discrepancia.
- Conserva los estilos de las filas disponibles y protege todas las fórmulas.

## v1.0.24.36 — inserción desde reportes o desde Atlas

- Permite elegir la pestaña destino y detecta automáticamente el rango real de filas.
- Acepta de una a ocho columnas destino; por ejemplo, `R,S,V,W` para impresiones, equivalentes, dúplex y atascos.
- Permite indicar directamente la columna de serie de la hoja maestra; por ejemplo, `B`.
- Puede importar la lectura más reciente de cada serie directamente desde la base de datos de Atlas.
- La base de datos puede usarse sola o combinarse con uno o varios reportes generados por Atlas.
- Mantiene el modo `AUTO`, la coincidencia exacta por número de serie, la protección de fórmulas, la vista previa y la generación de una copia nueva.
- Validado con `CONTADORES TORREON 2026.ods`: pestaña `Agosto`, serie `B`, localidad `M`, filas `2–390` y destinos `R,S,V,W`.

## v1.0.24.34 — columnas mensuales seleccionables

- El Insertador inteligente permite elegir el bloque de ocho columnas correspondiente al mes.
- Acepta un rango completo, por ejemplo `AK:AR`, o únicamente la primera columna, por ejemplo `AS`, que se normaliza a `AS:AZ`.
- Valida que siempre sean exactamente ocho columnas consecutivas y no permite exceder `XFD`.
- La selección invalida cualquier análisis anterior y debe confirmarse nuevamente en la vista previa.
- Los nombres de las celdas, mensajes, copia generada y reporte de discrepancias utilizan el rango realmente seleccionado.

## v1.0.24.33 — Separador por dependencia en Herramientas

- Añade el Separador de PDFs por dependencia al módulo de Herramientas auxiliares.
- Atlas entrega automáticamente al separador la ruta de su base de datos activa.
- Los scripts `.sh` externos se ejecutan explícitamente con Bash.
- El separador v0.1.5 utiliza el selector nativo de KDE mediante `kdialog`, coherente con Dolphin, y conserva el selector portátil como respaldo.

## v1.0.24.32 — separador PDF para lotes grandes

- Libera inmediatamente cada imagen de alta resolución después del OCR.
- Entrega los resultados a la interfaz en bloques pequeños con control de flujo.
- Evita la acumulación de miniaturas y señales cuando el análisis supera la velocidad de dibujo.
- Actualiza la tabla por lote y elimina el cálculo cuadrático de su altura.
- Conserva OCR, clasificación, corrección manual, exportación e historial existentes.

## v1.0.24.31 — acuerdo bilingüe de uso

- Presenta un acuerdo de licencia en español e inglés antes del primer uso de
  cada usuario del sistema operativo.
- Registra nombre, organización, usuario del sistema, equipo, fecha UTC,
  versión de Atlas y hash exacto del acuerdo aceptado.
- Una nueva versión o modificación del acuerdo exige una nueva aceptación.
- Declara expresamente que el Licenciatario conserva sus datos y que CodeCafe
  conserva Atlas; la aceptación no constituye una cesión del software.
- Añade una opción permanente en Ayuda para consultar ambas versiones.
- El registro se guarda fuera de SQLite y no modifica ni bloquea la base de
  datos operativa.

## v1.0.24.30 — captura de hostname desde informes HP

- Extrae el valor de `Nombre de host` mediante una lectura OCR dedicada del
  informe de configuración HP.
- Muestra el hostname como dato editable en la revisión individual y por lote.
- Al guardar contadores, completa el hostname del equipo solamente cuando el
  campo correspondiente está vacío.
- Nunca reemplaza un hostname existente y bloquea la asignación si el mismo
  hostname ya pertenece a otro equipo.
- Los equipos nuevos registrados desde el lote reciben también el hostname
  detectado cuando es único.
- Conserva la alta masiva de equipos y la navegación por teclado anteriores.

## v1.0.24.29 — alta masiva desde Registro de contadores

- Detecta números de serie del lote que todavía no existen en Directorio ni en
  Inventario.
- Solicita una sola confirmación para registrar todos los equipos nuevos del
  lote.
- Reutiliza la dependencia seleccionada en los datos comunes del reporte o
  permite elegirla una vez si todavía está sin especificar.
- Registra serie y modelo detectado como `Impresora / Activo`, sin inventar
  números de inventario, hostname o IP.
- Vincula la lectura nueva y las lecturas históricas huérfanas de la misma
  serie con el equipo recién creado.
- Permite conservar únicamente las lecturas o cancelar sin marcar el lote como
  guardado.
- Conserva íntegra la navegación por teclado restaurada en v1.0.24.28.

## v1.0.24.28 — restauración de navegación por teclado

- En la revisión ampliada del Registro de contadores, `Enter` vuelve a aplicar
  las correcciones y abrir el siguiente documento; en el último, aplica y
  cierra.
- `Flecha arriba` y `Flecha abajo` vuelven a recorrer exclusivamente los campos
  editables.
- En campos numéricos, las flechas ya no incrementan ni reducen accidentalmente
  el contador.
- Se conserva íntegra la funcionalidad de v1.0.24.27.

## v1.0.24.27 — mejoras operativas y biblioteca técnica

- Actualiza las horas de las órdenes al formato operativo de 24 horas `HH:mm:ss`.
- Añade un editor persistente para crear, editar y eliminar plantillas rápidas de Falla reportada.
- Permite renombrar de forma segura el PDF visible sin moverlo ni sobrescribir archivos.
- Añade una Biblioteca técnica independiente que indexa una carpeta configurable sin mover los originales.
- Añade Herramientas auxiliares para registrar y ejecutar las copias independientes de Data Bridge y del comparador de hojas.
- Mantiene intactos el control de calidad del Visor PDF, la base de datos y los demás módulos aprobados.

## v1.0.24.26 — corrección de cédula y horario

- Conserva el ajuste de texto configurado en la plantilla para las celdas laterales A53 y H53.
- Presenta y guarda las horas como `hh:mm:ss AM/PM`, manteniendo lectura de registros anteriores en formato de 24 horas.
